...
async function initDB(env) {
    try {
        // Check for table existence/correct schema by trying to select the new column
        ...
        if (e.message && (e.message.includes("no such column") || e.message.includes("no such table"))) {
            console.log("⚠️ Schema mismatch detected. Rebuilding tables...");
            await env.DB.exec("DROP TABLE IF EXISTS secrets");
        }
    }
    ...
    for (const q of queries) {
        await env.DB.prepare(q).run().catch(e => console.log("Init Warning:", e.message));
    }
}
...
export default {
    async fetch(request, env, ctx) {
        ...
        if (path.startsWith('/file/')) {
            if (!env.BUCKET) return new Response("R2 Not Configured", { status: 500 });
        ...
        if (path.startsWith('/api')) {
            try {
                ...
                if (path === '/api/upload' && method === 'POST') {
                     ...
                     const filename = request.headers.get("X-File-Name") || `file-${crypto.randomUUID()}`;
                     const key = `${crypto.randomUUID()}-${filename}`;
                     await env.BUCKET.put(key, request.body, { httpMetadata: { contentType: contentType } });
                ...
                if (path.match(/^\/api\/secret\/logs\/(.+)$/) && method === 'GET') {
                    ...
                    const logs = await safeSelectAll(env, "SELECT * FROM access_logs WHERE secret_id = ? ORDER BY viewed_at DESC", sid);
